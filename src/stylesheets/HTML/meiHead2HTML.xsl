...
<!-- doc -->
    ...
    <xsl:param name="proofreading" select="false()"/>
    <xsl:variable name="meiversion" select="root()/mei:mei/@meiversion"/>
    <xsl:variable name="meiversion.num" select="root()/mei:mei/@meiversion.num"/>
                ...
                <xsl:element name="html" namespace="http://www.w3.org/1999/xhtml">
                    <xsl:element name="head" namespace="http://www.w3.org/1999/xhtml">
                        <xsl:element name="title" namespace="http://www.w3.org/1999/xhtml">
                            <xsl:value-of select="concat(//mei:fileDesc/mei:title,'– HTML preview')"/>
                        </xsl:element>
                        <xsl:element name="link">
                            <xsl:attribute name="rel">stylesheet</xsl:attribute>
                            <xsl:attribute name="href">EdiromOnline-all.css</xsl:attribute>
                        </xsl:element>
                        <!--<xsl:element name="link">
                            <xsl:attribute name="rel">stylesheet</xsl:attribute>
                            <xsl:attribute name="href">todo.css</xsl:attribute>
                        </xsl:element>-->
                    </xsl:element>
                    <xsl:element name="body" namespace="http://www.w3.org/1999/xhtml">
                        <xsl:attribute name="class">headerViewContent</xsl:attribute>
                        <xsl:apply-templates/>
                    </xsl:element>
                </xsl:element>
            </xsl:when>
...
<!--    <xsl:template match="comment()" mode="#all"/>-->
    <xsl:template match="node()" mode="plainCommaSep">
        <xsl:choose>
            ...
            <!-- the node has descendant element nodes and attributes -->
            <xsl:when test="*/* and @*">
                <xsl:apply-templates select="@*" mode="plainCommaSep"/>
                <xsl:apply-templates select="*" mode="plainCommaSep"/>
            </xsl:when>
            <!-- the node has descendant element-nodes but no attribute -->
            <xsl:when test="*/* and not(@*)">
                <xsl:apply-templates select="@*" mode="plainCommaSep"/>
                <xsl:apply-templates select="*" mode="plainCommaSep"/>
            </xsl:when>
            <!-- the node has child elements and attributes -->
            <xsl:when test="* and @*">
                <xsl:apply-templates select="*" mode="plainCommaSep"/>
                <xsl:apply-templates select="@*" mode="plainCommaSep"/>
            </xsl:when>
            <!-- only element children that have attributes -->
            <xsl:when test="*/@*">
                <xsl:for-each select="*">
                    <xsl:apply-templates select="." mode="plainCommaSep"/>
          ...
          <!-- only element children without attributes -->
            ...
            <!-- if node is none of the above but has attributes -->
            <xsl:when test="@*">
                <xsl:text> (</xsl:text>
                <xsl:apply-templates select="@*" mode="plainCommaSep"/>
                <xsl:text>) </xsl:text>
...
<!-- TEMPLATEs ======================================================= -->
    ...
    <xsl:template match="mei:accessRestrict" mode="plainCommaSep">
        <xsl:apply-templates mode="#current"/>
    </xsl:template>
    ...
    <xsl:template match="mei:div"><?TODO take further markup in mei:head into account ?>
        <xsl:param name="sub" select="exists(parent::mei:div)"/>
        <xsl:param name="key" select="mei:head//text()"/>
    ...
    <xsl:template match="mei:div[exists(parent::mei:div)]" mode="plainCommaSep"><?TODO take further markup in mei:head into account ?>
        <xsl:param name="key" select="mei:head//text()"/>
        <xsl:call-template name="makeSubProperty">
            <xsl:with-param name="key" select="$key"/>
            <xsl:with-param name="node" select="."/>
    ...
    <xsl:template match="mei:div" mode="subProp">
        <xsl:apply-templates mode="subProp"/>
    </xsl:template>
    <xsl:template match="mei:head" mode="plainCommaSep">
        <?TODO check cases when not in history/div ?>
    </xsl:template>
    <xsl:template match="mei:head" mode="subProp">
        <?TODO check cases when not in history/div ?>
    </xsl:template>
    ...
    <xsl:template match="mei:work" name="work">
        ...
        <xsl:element name="div">
            ...
            <xsl:element name="div">
                ...
                <xsl:if test="mei:key or mei:meter or mei:mensuration or mei:tempo or mei:incip or mei:perfMedium or mei:castList">
                    <xsl:element name="div">
                        <xsl:attribute name="class">section</xsl:attribute>
                        <xsl:element name="h1">
                        ...
                        <xsl:element name="div">
                            <xsl:attribute name="class">propertyList</xsl:attribute>
                            <xsl:for-each select="mei:key | mei:meter | mei:mensuration | mei:tempo | mei:incip | mei:perfMedium | mei:castList">
                                <xsl:call-template name="makeProperty">
...
<!--                <xsl:apply-templates select="mei:componentGrp"/>-->
                <xsl:apply-templates select="mei:creation"/>
                <xsl:apply-templates select="mei:history"/>
                <xsl:apply-templates select="mei:seriesStmt"/>
                <xsl:apply-templates select="mei:classification"/>
                <xsl:apply-templates select="mei:otherChar"/>
                <xsl:apply-templates select="mei:expressionList"/>
...
<!--        <xsl:variable name="sub">-->
    ...
    <xsl:template match="mei:source" name="source">
        ...
        <xsl:apply-templates select="mei:history"/>
        <xsl:if test="./key or ./meter or ./perfMedium or ./castList or mei:componentGrp"><?TODO as in work ?>
            <xsl:element name="div">
                <xsl:attribute name="class">section</xsl:attribute>
                <xsl:element name="h1">
                    <xsl:value-of select="eof:getLabel('musicalInfo')"/>
    ...
    <xsl:template match="mei:title" name="title" mode="#default plainCommaSep subProp musicEncodingOrg">
        <xsl:param name="sub" tunnel="yes"/>
        <xsl:element name="div">
    ...
    <xsl:template match="mei:title" name="titleValue" mode="valueOnly">
    ...
    <xsl:template match="mei:respStmt" name="respStmt">
        ...
        <xsl:variable name="key">
            <xsl:choose>
                ...
                <xsl:when test="count(distinct-values(*/tokenize(@role,' '))) = 1"><?TODO support multiple values ?>
    ...
    <xsl:template match="mei:quote" mode="#all">
        <xsl:apply-templates mode="#current"/>
    </xsl:template>
    ...
    <xsl:template match="mei:identifier[ancestor::mei:p]" mode="plainCommaSep">
        <xsl:element name="span">
    ...
    <xsl:template match="mei:identifier" mode="valueOnly">
        <xsl:apply-templates mode="#current"/>
    </xsl:template>
    ...
    <xsl:template match="mei:availability[parent::mei:item]">
        <xsl:call-template name="makeProperty">
            <xsl:with-param name="node" select="."/>
        </xsl:call-template>
    </xsl:template>
...
<!-\\-          <xsl:with-param name="sub" select="true()"/>-\\->
  ...
  <!--<xsl:template match="mei:editionStmt">
    ...
    <xsl:template match="mei:geogName" name="geogName"><?TODO check propOrSub capabilities ?>
        <!--<xsl:element name="div">
            <xsl:attribute name="class">property</xsl:attribute>
            <xsl:element name="div">
                <xsl:attribute name="class">key</xsl:attribute>
...
<!--        <xsl:param name="key" select="local-name(.)"/>-->
    ...
    <xsl:template match="mei:dimensions" mode="subProp">
        <xsl:apply-templates select="node()" mode="valueOnly"/>
    </xsl:template>
    <xsl:template match="@label | @unit" mode="valueOnly"/>
...
<!--<xsl:template match="mei:p" mode="plainCommaSep">
    ...
    <xsl:template match="mei:rend" mode="#all">
        <xsl:variable name="style">
            ...
            <?TODO how to deal with @rend, translate to CSS in @style or to classes? ?>
        </xsl:variable>
        <xsl:element name="span">
    ...
    <xsl:template match="mei:plateNum">
        <xsl:call-template name="makeProperty">
            <xsl:with-param name="key">
                <xsl:choose>
                    <xsl:when test="@label">
    ...
    <xsl:template match="mei:plateNum" mode="plainCommaSep">
        <xsl:call-template name="makeSubProperty">
            <xsl:with-param name="node" select="."/>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="mei:captureMode">
        <xsl:call-template name="makeProperty">
            <xsl:with-param name="key">
                <xsl:choose>
                    <xsl:when test="@label">
    ...
    <xsl:template match="mei:captureMode" mode="plainCommaSep">
        <xsl:call-template name="makeSubProperty">
            <xsl:with-param name="node" select="."/>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="mei:perfDuration">
        <xsl:call-template name="makeProperty">
            <xsl:with-param name="key">
                <xsl:choose>
                    <xsl:when test="@label">
    ...
    <xsl:template match="mei:perfDuration" mode="plainCommaSep">
        <xsl:call-template name="makeSubProperty">
            <xsl:with-param name="node" select="."/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="mei:eventList">
        <xsl:variable name="key">
            <xsl:choose>
                ...
                <xsl:when test="mei:head">
                    <xsl:apply-templates select="mei:head" mode="valueOnly"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="eof:getLabel(local-name())"/>
                </xsl:otherwise>
    ...
    <xsl:template match="mei:eventList" mode="plainCommaSep">
        <xsl:param name="dated" select="true()"/>
        <xsl:choose>
            <xsl:when test="$dated">
                <xsl:element name="dl">
                    <xsl:for-each select="mei:event">
                        <xsl:element name="dt">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>[unsupported date]</xsl:otherwise>
                            </xsl:choose>
                        </xsl:element>
                        <xsl:element name="dd">
                            <xsl:apply-templates mode="plainCommaSep"/>
